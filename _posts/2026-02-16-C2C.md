...
2.  **Integer Overflow:**
    ...
    Maybe the logic is `hp += amount` (healing?) or the writeup implied the overflow makes the *result* negative. Use the provided writeup logic: "instantly killing the boss". i assume the cast results in massive damage or bypasses a check.
...
2.  **Attack Flow:**
    ...
    *  The admin has the flag note. i ideally want XSS in the *Admin's* session. But i can't inject notes there.
...
The application exposes raw `Next()` calls via the `/api/recent` endpoint. By collecting enough samples (55+), i can reconstruct the internal state and predict all future outputs.
...
3.  **Factorization:** With recovered parameters, i derive a relation $k/d \approx e/N^3$. **Continued Fractions** can efficiently recover the private exponent $d$, allowing factorization of $N$.
...
3.  **Known Plaintext:** The flag starts with `C2C{`. i scan the ciphertext for this pattern to find the exact keystream offset.
...
**Step 1: Extraction**
I located the start of the bundled JavaScript payload by inspecting the end of the binary (using `tail` or a hex editor). The payload starts with a large array and obfuscated logic.
...
By emulating the deobfuscation logic (running the extraction script in Node.js), I resolved the critical API calls:
*   `s(373, "rG]G")` -> `sulawesi`
*   `s(387)` -> `zstdCompress`
...
3.  **Code Execution:**
    ...
    *   i target the `Server::send_response` function, which is called at the end of the `process_put` handler.
    *   i construct a shellcode payload that reuses the existing socket (looping FDs 3-10) and spawns `/bin/sh`.
    *   i send a `PUT` request to `/proc/self/mem` targeting the address of `send_response` (Base + Offset).
...
3.  **Data Extraction:**
    ...
    *   i parsed the logs to extract the characters where the condition `!=` was false (i.e., NO sleep).
...
**Exploit:** i construct malicious strings (like `os.popen`) piece-by-piece using the string representations of `numpy` objects (e.g., `numpy.fix`, `numpy.typing`) and concatenating them. i then execute the payload to read the flag.
```python
