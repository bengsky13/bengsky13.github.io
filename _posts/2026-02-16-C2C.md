...
2.  **Integer Overflow:**
    ...
    *Correction:* The logic performs `hp -= amount`. If we send `2147483649` (int64), it becomes `-2147483647` (int32). `hp - (-large)` = `hp + large`. This heals him?
    *Re-analysis:* If the logic is `hp -= int32(amount)`, and we want to *reduce* HP. If we send a large positive `int64` that wraps to a large *positive* `int32`? No, max `int32` is $2^{31}-1$.
    ...
    *Correction based on Writeup:* "Sending 2^31 + 1... overflows to a large negative number... instantly killing the boss."
...
2.  **Attack Flow:**
    ...
    *   Window A opens Window B (Admin Panel - logged in as Admin). Loading this puts the sensitive flag in the DOM.
    *   Window A redirects Window B to `login` (Login CSRF), logging Window B into the **Attacker's** account.
    *   Window B loads the notes page. Since it's now the Attacker's account, it loads the keys/XSS note.
    *   XSS executes in Window B. It accesses `opener` (Window A? No, the exploit window opened Window B).
    *   Wait, if Window A opens Window B, `Window B.opener == Window A`. Window A does not have the flag.
    *   *Correction:* The XSS likely accesses the flag from *somewhere*. If the flag was loaded *before* the Login CSRF? No, the page reloads.
    *   *Refined Attack:* The admin has the flag note. We ideally want XSS in the *Admin's* session. But we can't inject notes there.
    ...
    *   *Check Writeup Logic:* "It opens Window 2... navigates to /api/notes to load the flag... Window 2 navigates to /... Window 3 performs Login CSRF... Window 2 reloads... loads Attacker's notes... XSS payload accesses opener...".
    *   Ah, the *history* or the `opener` context. If Window 1 (Attacker Site) opens Window 2 (Target Site), Window 2 is the victim.
    *   If Window 2 loads Flag, then navigates away, the DOM is gone.
    *   *Alternative:* IFrame? Embed Admin Notes in IFrame (if no X-Frame-Options), then navigate IFrame?
    ...
    *   So: Window 1 (Flag) opens Window 2 (Exploit/Attacker Account). Window 2 executes XSS, reads `opener` (Window 1).
    *   But Admin starts at Exploit Page. Exploit Page (Window 0) opens Window 1 (Target).
    *   Window 1 loads Flag.
    *   Window 1 navigates to Login -> Attacker Account. (Flag is lost from DOM).
    *   *Unless:* We use IFrames or multiple windows.
    *   *Solver Script:* `exploit()` returns `open('csrf')`. `csrf()` does form submit.
    *   Actually, the standard XS-Leak / Clobbering technique usually relies on `opener`.
    *   Let's trust the provided solver logic: The attacker sets up a chain where one window holds the flag and another (logged into attacker account) executes the XSS to read it.
