...
pcre.jit | 1 | INI_ALL | JIT compilation enabled by default on many builds. |
 - Long or adversarial inputs combined with quantified groups can cause the regex engine to perform huge amounts of work (backtracking or recursion). If those internal limits are exceeded, PCRE will fail and preg_match() will return FALSE instead of 1. Because the code uses if (preg_match(...)) directly, a FALSE (error) is treated as “no match” and the role check is effectively bypassed.
